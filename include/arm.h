...
static inline void debug() { asm volatile("nop"); }

static inline uint64 r_mpidr_el1()
{
    uint64 x;
...
static inline uint64 r_spsr_el1()
{
    uint64 x;
...
static inline void w_spsr_el1(uint64 spsr_el1)
{
    asm volatile("msr spsr_el1, %0" ::"r"(spsr_el1));
}
static inline uint64 r_elr_el1()
{
    uint64 x;
    asm volatile("mrs  %0,elr_el1" : "=r"(x));
    return x;
}
static inline void w_elr_el1(uint64 elr_el1)
{
    asm volatile("msr elr_el1, %0" ::"r"(elr_el1));
}

static inline uint64 r_sp_el0()
{
    uint64 x;
    asm volatile("mrs  %0,sp_el0" : "=r"(x));
    return x;
}
static inline void w_sp_el0(uint64 sp_el0)
{
    asm volatile("msr sp_el0, %0" ::"r"(sp_el0));
}
static inline uint64 r_ttbr0_el1()
{
    uint64 x;
    asm volatile("mrs  %0,ttbr0_el1" : "=r"(x));
    return x;
}
static inline void w_ttbr0_el1(uint64 ttbr0_el1)
{
    asm volatile("msr ttbr0_el1, %0" ::"r"(ttbr0_el1));
}

static inline void w_vbar_el1(uint64 x)
{
    asm volatile("msr vbar_el1, %0" ::"r"(x));
}
static inline uint64 r_vbar_el1(void)
{
    uint64 x;
    asm volatile("mrs %0,vbar_el1" : "=r"(x));
    return x;
}
static inline uint64 r_esr_el1(void)
{
    uint64 x;
    asm volatile("mrs %0,esr_el1" : "=r"(x));
    return x;
}
static inline int r_CurrentEL(void)
{
    uint64 x;
...
static inline uint64 r_cntpct_el0()
{
    uint64 x;
...
static inline uint64 r_cntfrq_el0()
{
    uint64 x;
    asm volatile("mrs %0,cntfrq_el0" : "=r"(x));
    return x;
}
static inline void w_cntp_cval_el0(uint64 cntp_cval_el0)
{
    asm volatile("msr cntp_cval_el0, %0" ::"r"(cntp_cval_el0));
}
static inline void w_cntp_ctl_el0(uint64 cntp_ctl_el0)
{
...
// Attr<n>, bits [8n+7:8n], for n = 7 to 0,  index is set by
// mair_el1[atr8|art7...|atr0]
//  atr0[7...0] Inner memory
//  attributes（Bits[3:0]）：影响核心内部缓存（L1/L2）行为;Outer memory
//  attributes（Bits[7:4]）：影响系统级缓存（共享缓存）行为
