                                ...
                                uint64 syscall_num)
{
    printf(
        ...
        "syscall_num:%d C-TF:%p curproc->tf->x8:%p initproc:tf->x8:%p\n",
        curproc->pid, TF, entry_error_messages[type], ((esr >> 26) & 0x3f),
        address, syscall_num, curproc->tf, curproc->tf->x8, initproc->tf->x8);
    debug();
    // panic("trap message");
}
