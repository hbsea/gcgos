...
#define HIGHT 0xFFFFFF8000000000
extern char end[];
int mmu_enabled = 0;
...
pagetable_t kvmmake(void)
{
    ...
    // 高半页表页映射
    printf("H pages map\n");
    // kvmmap(kpgtbl, HIGHT + (uint64)kpgtbl, (uint64)kpgtbl, PGSIZE, PTE_NORMAL | PTE_AP_RW_EL1);

    // uart registers
    printf("PL011\n");
    kvmmap(kpgtbl, HIGHT + PL011_BASE, PL011_BASE, PGSIZE, PTE_DEVICE | PTE_XN);
    ...
    kvmmap(kpgtbl, HIGHT + KERNBASE, KERNBASE, (uint64)etext - KERNBASE, PTE_NORMAL | PTE_AP_RO_EL1);
    kvmmap(kpgtbl, HIGHT + (uint64)etext, (uint64)etext, PHYSTOP - (uint64)etext, PTE_NORMAL | PTE_XN);
...
void kvminithart()
{
    ...
    uint64 ttbr1 = (uint64)kernel_pagetable;
