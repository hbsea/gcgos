...
pagetable_t kvmmake(void)
{
    ...
    // gic
    kvmmap(kpgtbl, GICD_BASE, GICD_BASE, 4 * PGSIZE,
    ...
    kvmmap(kpgtbl, (uint64)etext, (uint64)etext, PHYSTOP - (uint64)etext,
           PTE_NORMAL | PTE_XN);
    printf("trampoline:%p\n", trampoline);
    kvmmap(kpgtbl, TRAMPOLINE, (uint64)trampoline, 2 * PGSIZE,
           PTE_NORMAL | PTE_AP_RO);
...
{
    if (mappages(kpgtble, va, pa, sz, perm) != 0) panic("kvmmap");
}
...
void kvminithart()
{
    ...
    uint64 tcr = (TCR_T0SZ(25) | TCR_T1SZ(25) | TCR_TG0(0b00) | TCR_TG1(0b10) |
                  TCR_IPS(0));
    uint mair = ((MT_DEVICE_nGnRnE << (8 * AI_DEVICE_nGnRnE_IDX)) |
